% 3_D 4 ions Monte Carlo simulations
mp=1.67e-27;
m1=40*mp;
m2=2*mp;
k=1.38e-23;
...
% initial parameters
l=(Q*Q/(4*m*(w2(q))^2*pi*epsilon))^(1/3);
r10=[0,0,-1.4368*l];
r20=[0,0,-0.45438*l];
r30=[0,0,0.45438*l];
r40=[0,0,1.4368*l];
...
for k=1:N
    ...
    elseif r(k,1,3)>r(k,3,3)
        reorder(p,q)=reorder(p,q)+1;
        break;
    elseif r(k,1,3)>r(k,4,3)
         reorder(p,q)=reorder(p,q)+1;
        break;
    elseif r(k,2,3)>r(k,3,3)
         reorder(p,q)=reorder(p,q)+1;
        break;
    elseif r(k,2,3)>r(k,4,3)
         reorder(p,q)=reorder(p,q)+1;
        break;
    elseif r(k,3,3)>r(k,4,3)
         reorder(p,q)=reorder(p,q)+1;
        break;
    end
end

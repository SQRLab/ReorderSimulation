% 3_D 2 ions pseudopotential Monte Carlo simulations
mp=1.67e-27;
m1=40*mp;
m2=2*mp;
k=1.38e-23;
...
l2=(Q^2/(2*m1*w2(q)^2*pi*epsilon))^(1/3);
r10=[0,0,l2/2];
r20=-r10;
v10=vi(j,:);
v20=[0,0,0];
...
%     a(1,2,:)=-k*r(i,2,:)+kq*(r(i,2,:)-r(i,1,:))/(d(i))^3;
    a(1,1,1)=(-(w1(p)^2+w2(q)^2/2)/(q0/4)*cos(omega*t(i-1))+w2(q)^2/2)*r(i-1,1,1)+kq/m1*(r(i-1,1,1)-r(i-1,2,1))/(d(i))^3;
    a(1,1,2)=((w1(p)^2+w2(q)^2/2)/(q0/4)*cos(omega*t(i-1))+w2(q)^2/2)*r(i-1,1,2)+kq/m1*(r(i-1,1,2)-r(i-1,2,2))/(d(i))^3;
    a(1,1,3)=-w2(q)^2*r(i-1,1,3)+kq/m1*(r(i-1,1,3)-r(i-1,2,3))/(d(i))^3;
    a(1,2,1)=(-(w1(p)^2+w2(q)^2/2)/(q0/4)*cos(omega*t(i-1))+w2(q)^2/2)*r(i-1,2,1)+kq/m1*(r(i-1,2,1)-r(i-1,1,1))/(d(i))^3;
    a(1,2,2)=((w1(p)^2+w2(q)^2/2)/(q0/4)*cos(omega*t(i-1))+w2(q)^2/2)*r(i-1,2,2)+kq/m1*(r(i-1,2,2)-r(i-1,1,2))/(d(i))^3;
    a(1,2,3)=-w2(q)^2*r(i-1,2,3)+kq/m1*(r(i-1,2,3)-r(i-1,1,3))/(d(i))^3;
    v(i,:,:)=v(i-1,:,:)+a*tau;

...
function minDists(vf,vc)
    rid2 = 1e6 ; rii2 = 1e6 ; vid2 = 1e6 ; vii2 = 1e6
    Ni = length(vf[:,1]) ; Nc = length(vc[:,1])
    for i=1:Ni
        for j=i+1:Ni # check each pair of ions for distance and speed
        ...
        for j=1:Nc # check each ion-dipole pair for distance and speed
            r = vf[i,1]-vc[j,1] ; z=vf[i,2]-vc[j,2] ; vr = vf[i,3]-vc[j,3] ; vf[i,4]-vc[j,4]
            dist2 = r^2 + z^2
            v2 = vr^2 + vz^2
            if dist2<rid2
                vid2 = v2 ; rid2 = dist2                
    ...
    return sqrt(rid2),sqrt(rii2),sqrt(vid2),sqrt(vii2)
end

function collisionMode(rii,rid,a,e=0.1)
...
#                 println("Error, velocity above scope of laserInt") ; println("index found = ",first) ; println(" velocity = ",vil)
                first = length(nu[1,:])-1 # this assumes the cooling rates at the bounds are sufficiently close to zero
            elseif first < 1
#                 println("Error, velocity below scope of laserInt") ; println("index found = ",first) ; println(" velocity = ",vil)   
                first = 1 # this assumes the cooling rates at the bounds are sufficiently close to zero
            end
...
function runFasterCollision(vf,rc,zc,vrc,vzc,qc,mc,ac,Nt,dtSmall,Er,Ez,Nr,Nz,dr,dz,vl,I0,nul,dnul,nu0,A,Ti,dtLarge,dCutOut,dtMid,dCutIn,laserCool=true,eii = 0.01,eid = 0.01)
    ...
    for i=1:Nt       
        ...
        collision = collisionMode(rii,rid,vc[1,7],0.1)
        if collision
...
#             println("vf speeds after cooling = ",vf[1,3:4])
        ...
        for j=2:Ni
            if zs[j,i]>zs[j-1,i] && crossTest<1 # end sim if neighboring ions have reordered
                crossTest+=1
                print("Crossing at ",i," of ion ",j) 
                Nt = i+5000 # do 5000 more time steps after a crossing is detected
#                 break
            end
        end
#         if crossTest>0
...
# building a test function to add tweezers
function addTweezers(P0,alpha,w0r,w0z,lambda,r0,z0,dr,dz,Nrmid,Nzmid,inPlane=false,n=1.0) 
    """             
    ***
    Note that this approximates the tweezer force as an electric field that interacts with a monopole
    This means this function will not work accurately with multiple species and will have a non-zero impact on the 
    effects of collisions that coincide with the tweezed region
    ***
    This function takes the laser intensity, spot size in the radial and axial directions, 
    and the coupling constants to translate intensity to potential 
    and returns the effective monopole interacting electric fields this creates
    P0 is the laser power
    alpha is the polarizability of the particle for that wavelength, it is the real part only
    w0r and w0z are the 1/e beam diameters radially and axially respectively
    lambda is the wavelength for the tweezer beam
    r0 and z0 are the physical position from the simulation center for the tweezer beam center
    dr and dz are the size of the sim cells
    Nrmid and Nzmid are the centerpoint of the sim
    inPlane is true if the laser is propagating in the radial direction of the sim and false if it is perpendicular, defaults to false
    n is the index of refraction of the medium, defaults to vacuum 1.0
    """
    Er = np.zeros((Nr,Nz)) ; Ez = np.zeros((Nr,Nz)) ; Ey = np.zeros((Nr,Nz))  # initialize electric field arrays
    eps0 = 8.854e-12 ; c = 2.998e8 ; q=1.602e-19
    yR = pi*w0r*w0z/lambda
    C0 = -(2*P0/pi)*(alpha/(c*n*eps0))/q # cofactor for many of the terms
    
    r0 = ptovPos(r0,Nrmid,dr) # transform focal center to grid cell units
    z0 = ptovPos(z0,Nzmid,dz) # transform focal center to grid cell unit
    sigR = w0r/dr # transform 1/e diameter to grid cell units radius
    sigZ = w0z/dz # transform 1/e diameter to grid cell units radius
    yR = yR/dr # transform rayleigh range to grid cell units
    
    println("C0 = ",C0)
    println("Intensity = ",4*P0/(pi*w0r*w0z))
    println("sigR = ",sigR) ; println("sigZ = ",sigZ) ;  println("r0 = ",r0) ; println("z0 = ",z0) 
    # Here we truncate the added electric fields to 5 sigma of the laser intensity from the center point in either direction (does not truncate propagation direction)
    rlow = max(convert(UInt32,round(r0 - 5*sigR)),1) ; zlow = max(convert(UInt32,round(z0 - 5*sigZ)),1)
    rhigh = min(convert(UInt32,round(r0 + 5*sigR)),Nr) ; zhigh = min(convert(UInt32,round(z0 + 5*sigZ)),Nz)
    
    println("rlow = ",rlow) ; println("rhigh = ",rhigh)
    println("zlow = ",zlow) ; println("zhigh = ",zhigh)
    
    #debugging override
#     rlow = 1 ; rhigh = Nr ; zlow = 1 ; zhigh = Nz
    if inPlane # the radial direction is the focusing direction as in the beam travels in the plane of the sim along r
        for iCell=1:Nr
            for kCell=zlow:zhigh
                y = iCell ; r = 0 ; z = kCell
                Ez[iCell,kCell] = C0*exp( -((z-z0)^2/(2*sigZ^2)) )*(z-z0)*dz/( (dz^3*sigZ^3)*dr*sigR * (1 + (y^2/(2 * yR^2)))^2 )
                Ey[iCell,kCell] = 2*C0*exp( -((z-z0)^2/(2*sigZ^2)) )*(y-r0)*dr/( (dr^2*yR^2)*dr*sigR*dz*sigZ*(1 + (y^2/(2 * yR^2)))^2 ) 
            end
        end
        return Ey,Ez
    else # the radial direction is the radial focus of the laser beam as in the beam travels perpendicular to the sim
        for iCell=rlow:rhigh
            for kCell=zlow:zhigh
                y = 0 ; r = iCell ; z = kCell
                Er[iCell,kCell] = C0*exp( -((r-r0)^2/(2*sigR^2)) - ((z-z0)^2/(2*sigZ^2)) )*(r-r0)*dr/( (dr^3*sigR^3)*dz*sigZ * (1 + (y^2/(2 * yR^2)))^2 )
                Ez[iCell,kCell] = C0*exp( -((r-r0)^2/(2*sigR^2)) - ((z-z0)^2/(2*sigZ^2)) )*(z-z0)*dz/( (dz^3*sigZ^3)*dr*sigR * (1 + (y^2/(2 * yR^2)))^2 )
            end
        end
        return Er,Ez
    end
...
function classicalAlpha(w0list,w,Alist)
    """
    w0list is a list of relevant allowed transitions (relevant means close in frequency to w and relatively high decay rate)
    w is the applied frequency causing the polarization effect
    Alist is a list of the spontaneous decay rates for each of the w0 relevant transitions
    """
    q = 1.602e-19 ; m = 9.109e-31 ; eps0 = 8.854e-12 ; c = 2.998e8 ; # all constants in SI units
    Gamma = (q^2 * w^2)/( 6 * pi * eps0 * m * c^3 ) # classical damping rate from radiation
    alpha = 0.0
    N = length(w0list)
    for i=1:N
        w0 = w0list[i] ; Gamma = Alist[i] ;
        alpha += (q^2 / m) * (w0^2 - w^2 - im*w*Gamma)^-1
    end
    return alpha
end